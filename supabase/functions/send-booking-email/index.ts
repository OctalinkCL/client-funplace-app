...
Deno.serve(async (req) => {
  ...
  const formattedDate = new Date(booking.date + 'T12:00:00Z').toLocaleDateString('es-CL', {
    weekday: 'long', year: 'numeric', month: 'long', day: 'numeric',
  })
  const timeRange = `${booking.start_time.slice(0, 5)} – ${booking.end_time.slice(0, 5)}`

  // Get admin email
  const { data: adminUser } = await supabase.auth.admin.getUserById(space.admin_id)
  const adminEmail = adminUser?.user?.email

  const FROM = 'Funplace <notificaciones@funplace.cl>'
