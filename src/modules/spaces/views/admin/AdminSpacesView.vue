  ...
  <div>
    ...
    <div class="flex items-center justify-between mb-6">
      ...
      <div class="flex gap-2">
        <Button v-if="auth.hasModule('spaces')" @click="showCreateModal = true"
          >Crear espacio</Button
        >
        <Button
          v-if="auth.hasModule('services')"
          variant="outline"
    ...
    >
      ...
      <div class="flex gap-2">
        <Button v-if="auth.hasModule('spaces')" @click="showCreateModal = true"
          >Crear tu primer espacio</Button
        >
        <Button
          v-if="auth.hasModule('services')"
          variant="outline"
          @click="showCreateServiceModal = true"
          >Crear tu primer servicio</Button
        >
      </div>
    </div>
    ...
    <div v-else class="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
      <Card v-for="space in spaces" :key="space.id" class="shadow-none py-2">
        <CardContent class="px-2 grid gap-3">
          <!-- Thumbnail -->
          <div class="relative">
            <img
            ...
            <Badge
              ...
              :variant="space.is_published ? 'default' : 'secondary'"
            >
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent>
                ...
                <DropdownMenuItem
                  ...
                  "
                  >Disponibilidad</DropdownMenuItem
                >
                <DropdownMenuItem @click="togglePublish(space)">{{
                  space.is_published ? "Despublicar" : "Publicar"
                }}</DropdownMenuItem>
                <DropdownMenuItem @click="handleDelete(space)"
    ...
    />
    <ServiceCreateModal
      v-model:open="showCreateServiceModal"
...
const { spaces, loading, error, fetchSpaces, deleteSpace, togglePublish } =
  useSpaces();
...
const emptyMessage = computed(() => {
  ...
  if (hasSpaces && hasServices)
    return "Aún no tienes espacios ni servicios creados.";
